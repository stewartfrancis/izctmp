...
class _DataSetActionPlugin(ActionBase):
    def _run(self, ds_name, module_name, cics_data_sets_required, tmp=None, task_vars=None):
        ...
        try:
            _process_module_args(self, self.module_args, self._templar, ds_name, task_vars, cics_data_sets_required)
...
def _process_module_args(self, module_args, _templar, ds_name, task_vars, cics_data_sets_required):
    _process_region_data_set_args(self, module_args, _templar, ds_name, task_vars)
...
def _check_region_override(self, module_args, ds_name):
...
def _process_region_data_set_args(self, module_args, _templar, ds_name, task_vars):
    self.logger.error("Checking region overrides")
    if _check_region_override(self, module_args, ds_name):
        self.logger.error("Not overridden")
        pass
    elif _check_template(module_args, "region_data_sets"):
        self.logger.error("Applying template")
        self.logger.error(f"task vars: {task_vars}")
        self.logger.error(f"module_args: {module_args}")
        ...
        self.logger.error(f"updates: {updates}")

        module_args["region_data_sets"].update(updates)
    else:
